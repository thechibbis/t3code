...
export function makeOmpAdapter(ompSettings: OmpSettings, options?: OmpAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSession: OmpAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = ompModelSelection?.model
            ? resolveOmpAcpBaseModelId(ompModelSelection.model)
            : undefined;
          const boundModelId = yield* applyOmpAcpModelSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveOmpAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveOmpAcpBaseModelId(turnModelSelection.model)
                : undefined;
              const currentModelId = yield* applyOmpAcpModelSelection({
                runtime: ctx.acp,
              ...
              const displayModel = currentModelId
                ? resolveOmpAcpBaseModelId(currentModelId)
                : undefined;
    ...
    const respondToUserInput: OmpAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        yield* requireSession(threadId);
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "user-input/respond",
          detail: `omp does not support structured user-input requests: ${requestId}`,
        });
      });
